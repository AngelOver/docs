...
이 문서는 XHuoAPI의 프록시 서비스를 통해 VS Code에서 Claude Code 확장을 구성하고 사용하는 방법을 주로 설명합니다.
...
![VS Code Marketplace의 Claude Code 플러그인](https://cdn.xhuoapi.ai/10ddf71320.png)
...
플러그인을 처음 열면 Anthropic 공식 계정으로 로그인하라는 메시지가 표시됩니다. 우리는 공식 로그인을 건너뛰고 XHuoAPI의 프록시 API를 구성하여 사용할 수 있습니다. **공식 구독이 필요 없습니다.**
...
`~/.claude/settings.json`을 편집합니다 (존재하지 않으면 생성) `env` 구성을 추가하고 `{token}`을 XHuoAPI 콘솔에서 가져온 API 토큰으로 교체합니다:
...
> **팁**: API 토큰은 [XHuoAPI 콘솔](https://api.xhuoapi.ai/console)에서 확인할 수 있습니다. `ANTHROPIC_AUTH_TOKEN`의 값은 자동으로 `Bearer` 접두사가 추가되어 서버로 전송됩니다. 프로젝트 수준 구성은 사용자 수준 구성보다 우선합니다. 팀 프로젝트에서는 `.claude/settings.json` 공유 구성을 사용하는 것이 좋으며, 개인 민감 정보는 `.claude/settings.local.json`에 보관하세요.
...
![VS Code의 Claude Code 패널](https://cdn.xhuoapi.ai/b84b878420.jpg)
...
![VS Code의 Claude Code 대화 및提示](https://cdn.xhuoapi.ai/650377f5cc.png)
...
![VS Code 중의 Diff 비교 뷰](https://cdn.xhuoapi.ai/c42381ea9f.png)
...
2. API 토큰이 유효한지 확인 ( [콘솔](https://api.xhuoapi.ai/console)에서 확인 가능)
3. 네트워크 연결이 정상인지 확인하고 `https://api.xhuoapi.ai/v1`에 접근할 수 있는지 확인
