...
Podstawowym parametrem wejściowym tego API jest `audio_id`, który jest oficjalnym identyfikatorem utworu, zazwyczaj jest to identyfikator utworu z pełnym rozdzieleniem ścieżek (odpowiadający [generacji utworów](https://api.xhuoapi.ai/documents/suno-audios) z parametrem `action` ustawionym na `all_stems`). Opcjonalnie wspiera również `callback_url` jako adres do asynchronicznego wywołania zwrotnego.
