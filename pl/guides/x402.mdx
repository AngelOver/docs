---
title: "XHuoAPI x402 Przewodnik po integracji płatności"
description: "x402 集成指南 - XHuoAPI"
---

x402 to oparty na standardzie HTTP 402 Payment Required protokół płatności on-chain, który umożliwia wywołującym dokonanie rozliczenia on-chain w momencie inicjowania żądania API, co jest idealne dla scenariuszy rozliczeń na podstawie wywołań w programach automatycznych, agentach AI i mikroserwisach. Dzięki temu przewodnikowi możesz programowo zrealizować płatność za zamówienia XHuoAPI w swoim systemie, osiągając doświadczenie „żądanie = płatność”.
...
Dokument ten jest skierowany do deweloperów, którzy potrzebują zintegrować zdolności płatności x402 platformy XHuoAPI w swojej działalności, przedstawiając pełny proces od przygotowania środowiska do zakończenia wywołania płatności oraz dostarczając przykładowy kod w Axios, Fetch, Python requests i httpx. Kluczowe punkty:
...
- Wszystkie API znajdują się pod domeną `https://api.xhuoapi.ai`, nagłówek `Authorization` musi zawierać **Platform Token**.
...
Zaloguj się do konsoli [https://api.xhuoapi.ai](https://api.xhuoapi.ai), w liście zamówień lub na stronie szczegółów zamówienia możesz zobaczyć zamówienie, które wymaga płatności. Szczegóły zamówienia będą wyświetlać:
...
1. Otwórz stronę konsoli [https://api.xhuoapi.ai/console/platform-tokens](https://api.xhuoapi.ai/console/platform-tokens);
...
- Podstawowa domena API: `https://api.xhuoapi.ai`
...
- **Weryfikacja w konsoli**: Odwiedź stronę szczegółów zamówienia `https://api.xhuoapi.ai/console/orders/{order_id}`, jeśli strona wyświetla „płatność zakończona” lub status zamówienia zmienił się na opłacone/zakończone, oznacza to, że rozliczenie na łańcuchu zostało zakończone.
- **Weryfikacja API**: Wywołaj `GET https://api.xhuoapi.ai/api/v1/orders/{order_id}/` z nagłówkiem `Authorization: Bearer {platform_token}`, sprawdź pole `state` w odpowiedzi ( `PAID` lub `FINISHED` oznacza, że płatność zakończona pomyślnie).
...
- Zgłaszanie zgłoszeń i wsparcie klienta: [https://api.xhuoapi.ai/support](https://api.xhuoapi.ai/support)
- Wspólna komunikacja: Discord [https://discord.gg/f9GRuKCmRc](https://discord.gg/f9GRuKCmRc), X (Twitter) [https://x.com/XHuoAPI](https://x.com/XHuoAPI)
- Inne kanały: e-mail `office@xhuoapi.ai`, `office@germey.tech`; adres firmy 651 N Broad St, Suite 201, Middletown, Delaware, USA; obsługa WeChat, sprawdź najnowszy kod QR na stronie wsparcia.
