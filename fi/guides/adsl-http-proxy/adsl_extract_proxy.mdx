---
title: "ADSL Proxy API - Hakeminen ja käyttö"
description: "ADSL Direct Connection Proxy 集成指南 - XHuoAPI"
---

Tässä asiakirjassa esitellään ADSL Proxy API:n käyttöprosessi, jonka avulla voimme helposti poimia ADSL:n IP-osoitteet ja lisätä IP-valkoisen listan ADSL Proxyyn.

## Hakuprosessi

ADSL:n IP-osoitteiden poimimiseen ja ADSL Proxyyn IP-valkoisen listan lisäämiseen voit ensin siirtyä sivuille [ADSL Proxy Extract API](https://api.xhuoapi.ai/documents/d8426c17-d22c-449a-b6bf-0a7790928e4d) ja [ADSL Proxy Whitelist API](https://api.xhuoapi.ai/documents/c21534fe-1f62-424b-9b10-a805cb473dd1) ja napsauttaa "Acquire" -painiketta saadaksesi tarvittavat todisteet:
...
Ensimmäisellä hakemiskerralla saat ilmaisia krediittejä, joita voit käyttää ilmaiseksi.

## ADSL:n IP-osoitteiden poimiminen

Seuraavaksi voit täyttää vastaavat tiedot sivustolla [ADSL Proxy Extract API](https://api.xhuoapi.ai/documents/d8426c17-d22c-449a-b6bf-0a7790928e4d), ja seuraavaksi näytämme, kuinka poimia ADSL:n IP-osoitteet.

### Pyynnön otsikoiden ja pyynnön rungon asettaminen
...
Kun pyyntö onnistuu, API palauttaa vaihdettujen taustakuvien tulokset. Esimerkiksi:
...
Voimme nähdä, että tuloksessa on `data`-kenttä, joka sisältää yksityiskohtia tästä IP-tehtävästä, muut tiedot näkyvät alla olevassa kuvassa:

- `success` , tämä on ADSL:n IP-tehtävän tulosstatus.
- `data` , ADSL:n IP:n yksityiskohtaiset tiedot.

`data`-kentästä voimme selvästi nähdä tämän IP:n yksityiskohtaiset tiedot, pääasialliset tiedot ovat `ip`, joka osoittaa tämän kerran haetun IP-osoitteen, muut tiedot ovat seuraavat:

- `ip` , tämä on haettu ADSL:n IP-osoite.
- `port` , tämä on haettu ADSL:n IP-porttitiedot.
- `proxy` , haetun ADSL:n IP-välinetiedot.
- `province` , haetun ADSL:n IP:n maakunta.
- `city` , haetun ADSL:n IP:n kaupunki.

Voimme nähdä, että voimme helposti käyttää yllä olevaa `proxy` seuraaviin IP-välinetyöskentelyihin.

Ennen kuin käytämme IP-välinettä, meidän on ensin lisättävä oma verkkoyhteyden IP-osoite (hae paikallinen verkkoyhteyden IP-osoite itse) seuraavaan valkoiseen luetteloon, seuraavaksi kerromme, kuinka IP-välinettä käytetään:

### Välineen käyttö

> Huomio: Ennen kuin käytät haettua välinettä IP-osoitetta, varmista, että olet lisännyt IP-valkoisen luettelon, muuten et voi käyttää sitä, valkoisen luettelon lisäämistä koskevat ohjeet löytyvät alla olevasta sisällöstä.
...
Yllä olevista tuloksista voidaan nähdä, että saatu tulos vastaa haettua alueen tietoa, mikä osoittaa, että tämä ADSL IP-väline on onnistunut ja voimassa.

## ADSL-välinetyyppi `type`

Voimme myös asettaa ADSL:n IP-välinetyypin, joka jakautuu pääasiassa kahteen tyyppiin, ensimmäinen on `http`, toinen on `socks`, koska ensimmäinen on oletusarvoisesti käytössä, seuraavaksi käytämme toista esimerkkinä, voimme tehdä seuraavat asetukset.

Asetetaan IP-välinetyypiksi `socks`, tarkat asetukset näkyvät alla olevassa kuvassa:

<p><img src="https://cdn.xhuoapi.ai/h3xlwj.png" width="500" class="m-auto"></p>

Voit myös huomata, että oikealla puolella on vastaavat kutsukoodit, voit kopioida koodin suoraan suoritettavaksi tai voit suoraan napsauttaa "Kokeile" -painiketta testataksesi.
...
Ensimmäisellä kerralla, kun käytät tätä rajapintaa, sinun on täytettävä vähintään kolme tietoa: yksi on `authorization`, jonka voit valita suoraan pudotusvalikosta. Toinen parametri on `ip`, `ip` on se paikallinen verkkosoite, jota tarvitsemme aiemmin mainitusta IP-proxysta. Viimeinen parametri on `action`, `action` on se toimenpide, jota teemme valkoiselle listalle, tässä tapauksessa lisäämme sen.

Voit myös huomata, että oikealla puolella on vastaava kutsukoodin generointi, voit kopioida koodin suoraan suorittaaksesi sen tai voit suoraan napsauttaa "Try" -painiketta testataksesi.
...
Palautustuloksessa on yksi kenttä, joka esittelee seuraavaa:

- `success `, tämän toimenpiteen tuloksen tila.

Missä `success` sisältää toimenpiteen tuloksen tilan, voimme nähdä, että IP `113.112.122.132` on onnistuneesti lisätty valkoiseen listaan.
...
Tämän asiakirjan avulla olet oppinut, kuinka käyttää ADSL Proxy API:ta helposti ADSL:n IP:n hakemiseen ja ADSL Proxy:n IP-valkoisen listan lisäämiseen. Toivottavasti tämä asiakirja auttaa sinua paremmin integroimaan ja käyttämään tätä API:ta. Jos sinulla on kysyttävää, ota rohkeasti yhteyttä tekniseen tukitiimiimme.
