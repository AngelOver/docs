...
Tämän API:n keskeinen syöttöparametri on `audio_id`, joka on virallisesti generoitu kappaleen ID, joka yleensä on koko ääniraitojen erottelun kappaleen ID (vastaa [kappaleen generointia](https://platform.acedata.cloud/documents/suno-audios), jossa parametri `action` on `all_stems`). Valinnaisesti tuetaan myös `callback_url` -asynkronista palautusosoitetta.
