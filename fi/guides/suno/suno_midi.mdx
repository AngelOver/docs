---
title: "Suno MIDI Generation API -liittymisohjeet"
description: "Suno Music Generation 集成指南 - XHuoAPI"
---

SUNO sallii meidän tehdä toissijaisia luovia töitä generoimastamme musiikista ja saada musiikin MIDI-tiedostot. Tässä asiakirjassa selitetään liittymismenetelmät API:iin.

Tämän API:n keskeinen syöttöparametri on `audio_id`, joka on virallisesti generoitu kappaleen ID, joka yleensä on koko ääniraitojen erottelun kappaleen ID (vastaa [kappaleen generointia](https://api.xhuoapi.ai/documents/suno-audios), jossa parametri `action` on `all_stems`). Valinnaisesti tuetaan myös `callback_url` -asynkronista palautusosoitetta.

Tässä syötämme `audio_id`:n, joka on `c65e6ffd-ead3-4926-9c8c-a42ce202946b`.

```python
import requests

url = "https://api.xhuoapi.ai/v1/suno/midi"
...
Voit nähdä, että `data`-kentät ovat koko ääniraidan erottamisen jälkeen kappaleen MIDI-tietoja.
