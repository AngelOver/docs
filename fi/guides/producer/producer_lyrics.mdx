---
title: "Producer laulujen generointi API:n liittämisohjeet"
description: "Producer Music Generation 集成指南 - Ace Data Cloud"
---

Producer sallii meidän luoda laulujen sanoituksia vihjeiden avulla, tämä asiakirja selittää liittymismenetelmät API:iin.

Tässä API:ssa on vain yksi syöttöparametri, joka on `prompt`, se on laulujen sanoitusten luomisen vihje.

Tässä syötämme `prompt`-arvoksi `A song about winter`, joka on laulujen sanoitusten luomisen vihje.
...
Voimme nähdä, että `data`-kentän `lyrics` on luotujen laulujen sanoitusten sisältö.
