...
Detta dokument riktar sig till utvecklare som behöver integrera XHuoAPI-plattformens x402 betalningsfunktionalitet i sin verksamhet. Det beskriver hela processen från att förbereda miljön till att genomföra betalningsanrop och tillhandahåller exempel på Axios, Fetch, Python requests och httpx. Kärnpunkter:
...
- Alla API:er finns under domänen `https://api.xhuoapi.ai`, `Authorization`-huvudet måste innehålla **Platform Token**.
...
Logga in på konsolen [https://api.xhuoapi.ai](https://api.xhuoapi.ai), i beställningslistan eller beställningsdetaljsidan kan du se beställningen som behöver betalas. Beställningsdetaljerna visar:
...
1. Öppna konsolens sida [https://api.xhuoapi.ai/console/platform-tokens](https://api.xhuoapi.ai/console/platform-tokens);
...
- API grundläggande domän: `https://api.xhuoapi.ai`
...
- **Konsolverifiering**: Besök orderdetaljsidan `https://api.xhuoapi.ai/console/orders/{order_id}`. Om sidan visar "Betalning lyckades" eller orderstatus har ändrats till betald/avslutad, innebär det att kedjeavräkningen är klar.
- **API-verifiering**: Anropa `GET https://api.xhuoapi.ai/api/v1/orders/{order_id}/` och inkludera `Authorization: Bearer {platform_token}`, kontrollera `state`-fältet i svaret ( `PAID` eller `FINISHED` indikerar att betalningen lyckades).
...
- Skicka in ärenden och kundsupport: [https://api.xhuoapi.ai/support](https://api.xhuoapi.ai/support)
- Gemenskapsdiskussion: Discord [https://discord.gg/f9GRuKCmRc](https://discord.gg/f9GRuKCmRc), X (Twitter) [https://x.com/XHuoAPI](https://x.com/XHuoAPI)
- Andra kanaler: E-post `office@xhuoapi.ai`, `office@germey.tech`; företagsadress 651 N Broad St, Suite 201, Middletown, Delaware, USA; WeChat kundsupport, vänligen se den senaste QR-koden på supportsidan.
