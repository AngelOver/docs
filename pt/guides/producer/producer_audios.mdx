...
Para usar a API, você precisa primeiro acessar a página correspondente da [API de Geração de Áudios do Producer](https://api.xhuoapi.ai/documents/producer-audios) para solicitar o serviço correspondente. Após entrar na página, clique no botão "Adquirir", conforme mostrado na imagem:
...
Se você quiser continuar a música que enviou, pode definir o parâmetro `action` como `upload_extend` e inserir o ID da música que deseja continuar. O ID da música pode ser obtido usando a [API de Upload do Produtor](https://api.xhuoapi.ai/documents/7fd2cb52-772e-4cc4-a018-e2626ff6f447), conforme mostrado na imagem abaixo:
