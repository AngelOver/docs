---
title: "XHuoAPI x402 Guia de Integração de Pagamento"
description: "x402 集成指南 - XHuoAPI"
---

x402 é um protocolo de pagamento em blockchain baseado no padrão HTTP 402 Payment Required, que permite que o chamador complete a liquidação em blockchain ao iniciar uma solicitação de API, sendo muito adequado para cenários de cobrança por chamada entre programas automatizados, Agentes de IA e microsserviços. Através deste guia, você pode programaticamente realizar o pagamento de pedidos da XHuoAPI em seu próprio sistema, proporcionando uma experiência de "solicitação é pagamento".
...
Este documento é destinado a desenvolvedores que precisam integrar a capacidade de pagamento x402 da plataforma XHuoAPI em seus negócios, apresentando o fluxo completo desde a preparação do ambiente até a conclusão da chamada de pagamento, e fornecendo exemplos de código em Axios, Fetch, Python requests e httpx. Pontos principais:
...
- Todas as APIs estão localizadas sob o domínio `https://api.xhuoapi.ai`, e o cabeçalho `Authorization` deve incluir o **Token da Plataforma**.
...
Faça login no console [https://api.xhuoapi.ai](https://api.xhuoapi.ai), na lista de pedidos ou na página de detalhes do pedido você pode ver o pedido que precisa ser pago. Os detalhes do pedido mostrarão:
...
1. Abra a página do console [https://api.xhuoapi.ai/console/platform-tokens](https://api.xhuoapi.ai/console/platform-tokens);
...
- Domínio base da API: `https://api.xhuoapi.ai`
...
- **Verificação no console**: Acesse a página de detalhes do pedido `https://api.xhuoapi.ai/console/orders/{order_id}`, se a página mostrar "Pagamento bem-sucedido" ou o status do pedido tiver mudado para pago/concluído, isso significa que a liquidação na blockchain foi concluída.
- **Verificação da API**: Chame `GET https://api.xhuoapi.ai/api/v1/orders/{order_id}/` e inclua `Authorization: Bearer {platform_token}`, verifique o campo `state` na resposta ( `PAID` ou `FINISHED` indica pagamento bem-sucedido).
...
- Envio de tickets e suporte ao cliente: [https://api.xhuoapi.ai/support](https://api.xhuoapi.ai/support)
- Comunidade: Discord [https://discord.gg/f9GRuKCmRc](https://discord.gg/f9GRuKCmRc), X (Twitter) [https://x.com/XHuoAPI](https://x.com/XHuoAPI)
- Outros canais: E-mail `office@xhuoapi.ai`, `office@germey.tech`; endereço da empresa 651 N Broad St, Suite 201, Middletown, Delaware, EUA; para suporte via WeChat, consulte o código QR mais recente na página de suporte.
