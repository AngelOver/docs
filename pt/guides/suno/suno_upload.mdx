...
Com o ID da música, podemos usar a [Suno Audios Generation API](https://platform.acedata.cloud/documents/4da95d9d-7722-4a72-857d-bf6be86036e9) para gerar músicas personalizadas, por exemplo, passando `action` como `upload_extend` e `audio_id` como o ID da música retornado, podemos gerar uma nova música com base no áudio de referência.
