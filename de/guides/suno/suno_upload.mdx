...
Hier geben wir die `audio_url` als `https://cdn.xhuoapi.ai/suno_demo.mp3` ein, die eine öffentlich zugängliche CDN-Adresse ist.
...
Mit der Song-ID können wir die [Suno Audios Generation API](https://api.xhuoapi.ai/documents/4da95d9d-7722-4a72-857d-bf6be86036e9) verwenden, um benutzerdefinierte Songs zu generieren. Wenn wir beispielsweise `action` mit `upload_extend` übergeben und gleichzeitig `audio_id` mit der zurückgegebenen Song-ID übergeben, können wir basierend auf dem Referenzaudio einen neuen Song generieren.
