...
- 所有 API 均位於 `https://api.xhuoapi.ai` 域名下，`Authorization` 頭需攜帶 **帳戶令牌（Account Token）**。
...
登錄控制台 [https://api.xhuoapi.ai](https://api.xhuoapi.ai)，在訂單列表或訂單詳情頁可以看到需要支付的訂單。訂單詳情會展示：
...
1. 打開控制台頁面 [https://api.xhuoapi.ai/console/platform-tokens](https://api.xhuoapi.ai/console/platform-tokens)；
...
- **控制台驗證**：訪問訂單詳情頁 `https://api.xhuoapi.ai/console/orders/{order_id}`，若頁面顯示「支付成功」或訂單狀態已變為已支付/已完成，即代表鏈上結算完成。
- **API 驗證**：調用 `GET https://api.xhuoapi.ai/api/v1/orders/{order_id}/` 並攜帶 `Authorization: Bearer {platform_token}`，檢查響應中的 `state` 欄位（`PAID` 或 `FINISHED` 表示支付成功）。
...
- 提交工單與客服支持：[https://api.xhuoapi.ai/support](https://api.xhuoapi.ai/support)
- 社區交流：Discord [https://discord.gg/f9GRuKCmRc](https://discord.gg/f9GRuKCmRc)，X（Twitter）[https://x.com/XHuoAPI](https://x.com/XHuoAPI)
- 其他渠道：郵箱 `office@xhuoapi.ai`、`office@germey.tech`；公司地址 651 N Broad St, Suite 201, Middletown, Delaware, USA；WeChat 客服請在支持頁面查看最新二維碼。
