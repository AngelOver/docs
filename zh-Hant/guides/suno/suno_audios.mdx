...
然而 Suno 官方並沒有提供 API 的，XHuoAPI 提供了一套 Suno 的 API，模擬對接了 Suno 官方，可以方便快捷地生成想要的音樂。

## 申請和使用

要使用 Suno Audios API，首先可以到 [Suno Audios Generation API](https://api.xhuoapi.ai/documents/4da95d9d-7722-4a72-857d-bf6be86036e9) 頁面點擊「Acquire」按鈕，獲取請求所需要的憑證：
...
> 注意，這裡的歌詞中 `\n` 是換行符，如果你不知道如何生成歌詞，可以使用 XHuoAPI 提供的歌詞生成 API 來通過 prompt 生成歌詞，API 是 [Suno Lyrics Generation API](https://api.xhuoapi.ai/documents/514d82dc-f7ab-4638-9f21-8b9275916b08)。
...
如果想对自己上传的歌曲进行继续生成的话，可以将参数 `action` 设置为 `upload_extend` ，并且输入需要继续生成自定义上传的歌曲 ID，歌曲 ID 的获取是使用 [Suno Upload Generation API](https://api.xhuoapi.ai/documents/766db278-012c-43c4-9245-5f18d8dc4d82)来获取，如下图所示：
...
2025 年 8 月份 suno 新出 Add Insterumental 功能，首先需要上傳一首清唱無配音的歌曲, 讓 suno 幫你配樂，首先可以先到 [Suno Upload API](https://api.xhuoapi.ai/documents/766db278-012c-43c4-9245-5f18d8dc4d82)上傳一首清唱無配樂的歌曲，對應如下如的操作如下圖所示：
...
2025 年 8 月份 suno 新出 Add Vocals 功能，首先需要上傳一首純音樂，讓 suno 填詞、出人聲歌唱，首先可以先到 [Suno Upload API](https://api.xhuoapi.ai/documents/766db278-012c-43c4-9245-5f18d8dc4d82)上傳一首清唱無配樂的歌曲，對應如下如的操作如下圖所示：
