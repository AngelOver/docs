...
有了聲音角色 ID 之後，我們便可以使用 [Suno Audios Generation API](https://api.xhuoapi.ai/documents/4da95d9d-7722-4a72-857d-bf6be86036e9) 來進行音樂生成了。將 `action` 設為 `generate`，並將 `persona_id` 設為上面返回的聲音角色 ID，生成的歌曲將使用克隆的聲音進行演唱。
