...
本文档主要介绍如何通过 XHuoAPI 的代理服务，在终端中安装、配置和使用 Claude Code CLI。
...
安装完成后，默认会要求你登录 Anthropic 官方账号。我们可以通过配置环境变量来使用 XHuoAPI 的代理 API，**无需官方订阅**。
...
将以下内容添加到你的 Shell 配置文件（`~/.zshrc`、`~/.bashrc` 或 `~/.bash_profile`）中，将 `{token}` 替换为你在 XHuoAPI 控制台获取的 API 令牌：
...
> **提示**：API 令牌可在 [XHuoAPI 控制台](https://api.xhuoapi.ai/console) 中查看。`ANTHROPIC_AUTH_TOKEN` 的值会被自动添加 `Bearer` 前缀发送到服务端。
...
### 连接失败怎么办？
1. 檢查 `~/.claude/config.json` 文件是否正確創建，內容為 `{"primaryApiKey": "self"}`
2. 確認環境變量已正確設置：
   ```bash
...
如果安裝後提示 `command not found`，請嘗試：

1. 關閉並重新打開終端
2. 檢查 PATH：`echo $PATH`
3. 重新安裝：`curl -fsSL https://claude.ai/install.sh | bash`
...
- 📖 [Claude Code CLI 官方文檔](https://code.claude.com/docs/en/quickstart)
- 📋 [CLI 命令參考](https://code.claude.com/docs/en/cli-reference)
- 🔧 [XHuoAPI Claude Code 服務](https://api.xhuoapi.ai/documents/claude-messages)
- 💬 如有任何問題，歡迎通過平台客服與我們聯繫
