...
有了歌曲 ID 之後，我們便可以使用 [Producer Audios Generation API](https://api.xhuoapi.ai/documents/producer-audios-integration) 來進行自定義歌曲生成了，比如說 `action` 傳入 `upload_extend`，同時 `audio_id` 傳入返回的歌曲 ID，就可以根據參考音頻生成新的歌曲了。
