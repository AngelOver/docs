...
Questo documento è rivolto agli sviluppatori che necessitano di integrare la capacità di pagamento x402 della piattaforma XHuoAPI nel proprio business, introducendo il processo completo dalla preparazione dell'ambiente al completamento della chiamata di pagamento, e fornendo esempi di codice per Axios, Fetch, Python requests e httpx. Punti chiave:
...
- Tutte le API si trovano sotto il dominio `https://api.xhuoapi.ai`, l'intestazione `Authorization` deve contenere il **Platform Token**.
...
Accedi alla console [https://api.xhuoapi.ai](https://api.xhuoapi.ai), nella lista degli ordini o nella pagina dei dettagli dell'ordine puoi vedere l'ordine da pagare. I dettagli dell'ordine mostreranno:
...
1. Apri la pagina della console [https://api.xhuoapi.ai/console/platform-tokens](https://api.xhuoapi.ai/console/platform-tokens);
...
- Dominio base API: `https://api.xhuoapi.ai`
...
- **Verifica della console**：Visita la pagina dei dettagli dell'ordine `https://api.xhuoapi.ai/console/orders/{order_id}`. Se la pagina mostra "Pagamento riuscito" o lo stato dell'ordine è cambiato in pagato/completato, significa che il regolamento sulla catena è completato.
- **Verifica API**：Chiama `GET https://api.xhuoapi.ai/api/v1/orders/{order_id}/` e porta `Authorization: Bearer {platform_token}`, controlla il campo `state` nella risposta ( `PAID` o `FINISHED` indica pagamento riuscito).
...
- Invia un ticket e supporto clienti: [https://api.xhuoapi.ai/support](https://api.xhuoapi.ai/support)
- Comunicazione nella comunità: Discord [https://discord.gg/f9GRuKCmRc](https://discord.gg/f9GRuKCmRc), X (Twitter) [https://x.com/XHuoAPI](https://x.com/XHuoAPI)
- Altri canali: email `office@xhuoapi.ai`, `office@germey.tech`; indirizzo aziendale 651 N Broad St, Suite 201, Middletown, Delaware, USA; per il supporto WeChat controlla il codice QR più recente nella pagina di supporto.
