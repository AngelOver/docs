...
[MCP（Model Context Protocol）](https://modelcontextprotocol.io) は Anthropic が開発したモデルコンテキストプロトコルで、Claude や GPT などの AI モデルが標準化されたインターフェースを通じて外部ツールを呼び出すことを可能にします。XHuoAPI が提供する Flux MCP Server を利用することで、Claude Desktop、VS Code、Cursor などの AI クライアント上で直接 AI 画像の生成や編集が行えます。
...
2. [Flux Images API](https://api.xhuoapi.ai/documents/flux-images) ページへ移動
...
- GitHub リポジトリ：[XHuoAPI/FluxMCP](https://github.com/XHuoAPI/FluxMCP)
- PyPI パッケージ：[mcp-flux-pro](https://pypi.org/project/mcp-flux-pro/)
- API ドキュメント：[Flux 画像生成 API](https://api.xhuoapi.ai/documents/flux)
