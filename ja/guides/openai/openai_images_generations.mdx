...
OpenAI Images Generations API を使用するには、まず [OpenAI Images Generations API](https://api.xhuoapi.ai/documents/fd932485-90c7-45d6-8394-1e14b6f07b2b) ページで「Acquire」ボタンをクリックして、リクエストに必要な認証情報を取得してください：
...
呼び出し方法は他のモデルと完全に同じで、`model` フィールドに `gpt-image-2` を指定するだけです。返却される `url` は `platform.cdn.xhuoapi.ai` に永久ホスティングされた画像リンクで、ブラウザで直接開くかウェブページに埋め込むことができます。
