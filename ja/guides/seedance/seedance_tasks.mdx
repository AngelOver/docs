...
SeeDance Tasks API を使用するには、まず申請ページ [SeeDance Video Generation API](https://api.xhuoapi.ai/documents/seedance-videos) で該当するサービスを申請し、その後、SeeDance Video Generation API のタスクIDをコピーします。以下の図のように：

<p><img src="https://cdn.xhuoapi.ai/om6p6g.png" width="500" className="m-auto" /></p>

最後に、Tasks API ページ [SeeDance Tasks API](https://api.xhuoapi.ai/documents/seedance-tasks) に移動し、該当するサービスを申請します。ページに入ったら、「Acquire」ボタンをクリックします。以下の図のように：
...
SeeDance Tasks API は、SeeDance Video Generation API の結果を照会するために使用できます。SeeDance Video Generation API の使用方法については、文書 [SeeDance Video Generation API](https://api.xhuoapi.ai/documents/seedance-videos-integration) を参照してください。
