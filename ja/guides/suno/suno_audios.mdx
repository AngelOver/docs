...
しかし、Suno公式はAPIを提供していません。XHuoAPIはSunoのAPIを提供しており、Suno公式に接続を模倣して、希望する音楽を簡単かつ迅速に生成できます。

## 申請と使用

Suno Audios APIを使用するには、まず[Suno Audios Generation API](https://api.xhuoapi.ai/documents/4da95d9d-7722-4a72-857d-bf6be86036e9)ページにアクセスし、「Acquire」ボタンをクリックして、リクエストに必要な認証情報を取得します：
...
> 注意：ここでの歌詞中の `\n` は改行文字です。歌詞を生成する方法がわからない場合は、XHuoAPIが提供する歌詞生成APIを使用して、プロンプトを通じて歌詞を生成できます。APIは [Suno Lyrics Generation API](https://api.xhuoapi.ai/documents/514d82dc-f7ab-4638-9f21-8b9275916b08) です。
...
## カスタム歌手スタイル生成機能
```
もし歌手のスタイルを使用して曲を生成したい場合、まずは上記の基本的な使用法に従って曲を生成します。最後に、この曲を歌手のスタイルに設定する必要があり、[Suno Persona API](https://api.xhuoapi.ai/documents/78bb6c62-6ce0-490f-a7df-e89d80ec0583)にアクセスして、公式に生成された音楽 ID `audio_id` を使用して歌手スタイルの id パラメータ `persona_id` を生成する必要があります。具体的なパラメータは以下の図の通りです：

<p><img src="https://cdn.xhuoapi.ai/pmzo3l.png" width="500" class="m-auto"></p>

記入が完了した後、自動生成されたコードは以下の通りです：

<p><img src="https://cdn.xhuoapi.ai/a5g0nj.png" width="500" class="m-auto"></p>
...
<p><img src="https://cdn.xhuoapi.ai/fukijq.png" width="500" class="m-auto"></p>

記入が完了した後、自動生成されたコードは以下の通りです：

<p><img src="https://cdn.xhuoapi.ai/5uzk9d.png" width="500" class="m-auto"></p>
...
自分がアップロードした曲を続けて生成したい場合、パラメータ `action` を `upload_extend` に設定し、続けて生成するカスタムアップロードの曲 ID を入力します。曲 ID の取得は [Suno Upload Generation API](https://api.xhuoapi.ai/documents/766db278-012c-43c4-9245-5f18d8dc4d82) を使用して行います。以下の図の通りです：

<p><img src="https://cdn.xhuoapi.ai/a0mn5e.png" width="500" class="m-auto"></p>
...
<p><img src="https://cdn.xhuoapi.ai/zp9s42.png" width="500" class="m-auto"></p>

記入が完了した後、自動生成されたコードは以下の通りです：

<p><img src="https://cdn.xhuoapi.ai/wwpw78.png" width="500" class="m-auto"></p>
...
2025年8月にSunoは新しいAdd Insterumental機能を発表しました。まず、無伴奏の清唱曲をアップロードし、Sunoに伴奏を付けてもらう必要があります。まず、[Suno Upload API](https://api.xhuoapi.ai/documents/766db278-012c-43c4-9245-5f18d8dc4d82)にアクセスして、無伴奏の清唱曲をアップロードします。操作は以下の図の通りです：
...
2025年8月にsunoが新たに追加したボーカル追加機能では、まず純粋な音楽をアップロードし、sunoに歌詞を作成させ、ボーカルを歌わせる必要があります。まずは[Suno Upload API](https://api.xhuoapi.ai/documents/766db278-012c-43c4-9245-5f18d8dc4d82)にアクセスして、アカペラの無伴奏曲をアップロードします。対応する操作は以下の図のようになります：
