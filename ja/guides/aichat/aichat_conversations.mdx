...
XHuoAPIが提供するAI Q&A APIは、上記の状況に対して最適化されており、Q&Aの効果を変えずに連続対話の実装をカプセル化しています。接続時に`messages`の渡し方を気にする必要はなく、トークンの制限を超える問題も心配する必要はありません（API内部で自動的に処理されます）。また、対話のクエリ、修正などの機能も提供されており、全体の接続が大幅に簡素化されています。

この文書では、AI Q&A APIの接続説明を紹介します。

## 申請プロセス

APIを使用するには、まず[AI Q&A API](https://api.xhuoapi.ai/documents/59fb1199-6694-4afb-a222-3554d7f7d05a)の該当ページでサービスを申請する必要があります。ページに入ったら、「Acquire」ボタンをクリックします。以下の図のように：
...
その画像のリンクは [https://cdn.xhuoapi.ai/ht05g0.png](https://cdn.xhuoapi.ai/ht05g0.png) で、これを `references` パラメータとして渡すだけです。同時に、モデルは視覚認識をサポートするモデルを選択する必要があります。現在サポートされているのは `gpt-4-vision` ですので、入力は以下の通りです：
