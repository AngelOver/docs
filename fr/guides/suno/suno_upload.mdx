...
Avec l'ID de la chanson, nous pouvons utiliser l'[API de génération de chansons Suno](https://api.xhuoapi.ai/documents/4da95d9d-7722-4a72-857d-bf6be86036e9) pour générer des chansons personnalisées, par exemple, en passant `action` comme `upload_extend`, tout en passant `audio_id` comme l'ID de la chanson retourné, nous pouvons générer une nouvelle chanson basée sur l'audio de référence.
