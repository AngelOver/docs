...
Le paramètre d'entrée principal de cette API est `audio_id`, qui est l'ID de la chanson générée par l'officiel, généralement l'ID de la chanson avec toutes les pistes séparées (correspondant au [génération de chanson](https://api.xhuoapi.ai/documents/suno-audios) où le paramètre `action` est `all_stems`). Il est également possible de fournir une adresse de rappel asynchrone `callback_url`.
