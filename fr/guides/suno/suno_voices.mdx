...
Une fois que vous disposez de l'ID du personnage vocal, vous pouvez utiliser la [Suno Audios Generation API](https://api.xhuoapi.ai/documents/4da95d9d-7722-4a72-857d-bf6be86036e9) pour générer de la musique. Définissez `action` sur `generate` et `persona_id` sur l'ID du personnage vocal retourné précédemment. La chanson générée sera chantée avec la voix clonée.
