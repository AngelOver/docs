...
Ce document s'adresse aux développeurs souhaitant intégrer la capacité de paiement x402 de la plateforme XHuoAPI dans leurs activités, en présentant le processus complet depuis la préparation de l'environnement jusqu'à l'appel de paiement, et en fournissant des exemples de code pour Axios, Fetch, Python requests et httpx. Points clés :
...
- Toutes les API se trouvent sous le domaine `https://api.xhuoapi.ai`, l'en-tête `Authorization` doit inclure le **Token de Plateforme**.
...
Connectez-vous à la console [https://api.xhuoapi.ai](https://api.xhuoapi.ai), dans la liste des commandes ou la page de détails de la commande, vous pouvez voir les commandes à payer. Les détails de la commande afficheront :
...
1. Ouvrez la page de la console [https://api.xhuoapi.ai/console/platform-tokens](https://api.xhuoapi.ai/console/platform-tokens) ;
...
- Domaine de base de l'API : `https://api.xhuoapi.ai`
...
- **Vérification dans la console** : Accédez à la page de détails de la commande `https://api.xhuoapi.ai/console/orders/{order_id}`, si la page affiche « paiement réussi » ou que l'état de la commande est devenu payé/terminé, cela signifie que le règlement sur la chaîne est terminé.
- **Vérification API** : Appelez `GET https://api.xhuoapi.ai/api/v1/orders/{order_id}/` avec `Authorization: Bearer {platform_token}`, vérifiez le champ `state` dans la réponse ( `PAID` ou `FINISHED` indique un paiement réussi).
...
- Soumettre un ticket et support client : [https://api.xhuoapi.ai/support](https://api.xhuoapi.ai/support)
- Échanges communautaires : Discord [https://discord.gg/f9GRuKCmRc](https://discord.gg/f9GRuKCmRc), X (Twitter) [https://x.com/XHuoAPI](https://x.com/XHuoAPI)
- Autres canaux : e-mail `office@xhuoapi.ai`, `office@germey.tech` ; adresse de l'entreprise 651 N Broad St, Suite 201, Middletown, Delaware, USA ; pour le service client WeChat, veuillez consulter le dernier code QR sur la page de support.
