---
title: "دليل دمج الدفع XHuoAPI x402"
description: "x402 集成指南 - XHuoAPI"
---

x402 هو بروتوكول دفع على السلسلة يعتمد على معيار HTTP 402 Payment Required، مما يسمح للجهات المستدعية بإجراء التسوية على السلسلة أثناء تقديم طلب API، وهو مثالي لسيناريوهات الفوترة حسب الطلب بين البرامج الآلية، ووكيل الذكاء الاصطناعي، والخدمات الصغيرة. من خلال هذا الدليل، يمكنك إتمام دفع طلبات XHuoAPI برمجيًا في نظامك، مما يحقق تجربة "طلب وادفع".
...
تستهدف هذه الوثيقة المطورين الذين يحتاجون إلى دمج قدرة الدفع x402 من منصة XHuoAPI في أعمالهم، وتقدم العملية الكاملة من إعداد البيئة إلى إتمام استدعاء الدفع، مع توفير أمثلة على Axios وFetch وPython requests وhttpx. النقاط الأساسية:
...
- جميع واجهات برمجة التطبيقات موجودة تحت اسم النطاق `https://api.xhuoapi.ai`، ويجب أن يتضمن رأس `Authorization` **رمز المنصة**.
...
قم بتسجيل الدخول إلى وحدة التحكم [https://api.xhuoapi.ai](https://api.xhuoapi.ai)، في قائمة الطلبات أو صفحة تفاصيل الطلب يمكنك رؤية الطلبات التي تحتاج إلى دفع. ستظهر تفاصيل الطلب:
...
1. افتح صفحة وحدة التحكم [https://api.xhuoapi.ai/console/platform-tokens](https://api.xhuoapi.ai/console/platform-tokens)؛
...
- اسم النطاق الأساسي لواجهة برمجة التطبيقات: `https://api.xhuoapi.ai`
...
- **التحقق من وحدة التحكم**: زيارة صفحة تفاصيل الطلب `https://api.xhuoapi.ai/console/orders/{order_id}`، إذا كانت الصفحة تعرض "نجح الدفع" أو تغيرت حالة الطلب إلى مدفوعة/مكتملة، فهذا يعني أن التسوية على السلسلة قد اكتملت.
- **التحقق من API**: استدعاء `GET https://api.xhuoapi.ai/api/v1/orders/{order_id}/` مع تضمين `Authorization: Bearer {platform_token}`، تحقق من حقل `state` في الاستجابة ( `PAID` أو `FINISHED` تعني نجاح الدفع).
...
- تقديم تذكرة ودعم العملاء: [https://api.xhuoapi.ai/support](https://api.xhuoapi.ai/support)
- التواصل المجتمعي: Discord [https://discord.gg/f9GRuKCmRc](https://discord.gg/f9GRuKCmRc)، X (Twitter) [https://x.com/XHuoAPI](https://x.com/XHuoAPI)
- قنوات أخرى: البريد الإلكتروني `office@xhuoapi.ai`، `office@germey.tech`؛ عنوان الشركة 651 N Broad St, Suite 201, Middletown, Delaware, USA؛ يرجى مراجعة صفحة الدعم للحصول على أحدث رمز QR لخدمة WeChat.
