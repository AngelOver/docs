...
تتناول هذه الوثيقة كيفية تكوين واستخدام Claude Code GitHub Actions من خلال خدمة الوكيل من XHuoAPI.
...
![صفحة تثبيت تطبيق GitHub Claude](https://cdn.xhuoapi.ai/ba4b8df66d.png)
...
قم بإضافة مفتاح API الخاص بـ XHuoAPI كسر للمستودع:

1. انتقل إلى **Settings** → **Secrets and variables** → **Actions**
2. انقر على **New repository secret**
3. اكتب في Name `ANTHROPIC_API_KEY`، وأدخل قيمة رمز API الذي حصلت عليه من XHuoAPI
4. انقر على **Add secret** للحفظ

> **تلميح**: يمكنك عرض رمز API في [وحدة تحكم XHuoAPI](https://api.xhuoapi.ai/console).
...
إذا كنت بحاجة إلى استخدام نقطة نهاية API الخاصة بـ XHuoAPI، فستحتاج أيضًا إلى إعداد متغيرات البيئة في Workflow:
...
قم بتسجيل الدخول إلى [لوحة تحكم XHuoAPI](https://api.xhuoapi.ai/console) لرؤية الرصيد المتبقي الحالي وحالة الاستخدام.
...
- 🔧 [خدمة Claude Code من XHuoAPI](https://api.xhuoapi.ai/documents/claude-messages)
