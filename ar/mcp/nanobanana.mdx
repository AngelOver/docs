---
title: "دليل تكامل Nano Banana MCP"
description: "nanobanana MCP 服务器集成"
---

[MCP (نموذج بروتوكول السياق)](https://modelcontextprotocol.io) هو بروتوكول سياق النموذج الذي أطلقته شركة Anthropic، والذي يسمح لنماذج الذكاء الاصطناعي (مثل Claude وGPT وغيرها) باستدعاء أدوات خارجية من خلال واجهات موحدة. من خلال خادم Nano Banana MCP المقدم من XHuoAPI، يمكنك مباشرةً إنشاء وتحرير صور الذكاء الاصطناعي في عملاء الذكاء الاصطناعي مثل Claude Desktop وVS Code وCursor.

## نظرة عامة على الوظائف

يوفر خادم Nano Banana MCP الوظائف الأساسية التالية:

- **توليد الصور** — توليد صور عالية الجودة من خلال نصوص توجيهية
- **تحرير الصور** — تعديل الصور الموجودة أو دمج عدة صور
- **تجربة الملابس الافتراضية** — ارتداء الملابس على صور الأشخاص
- **زرع المنتجات** — وضع المنتجات في مشاهد حقيقية
- **دعم نماذج متعددة** — دعم نماذج nano-banana وnano-banana-2 وnano-banana-pro
- **استعلام المهام** — مراقبة تقدم التوليد والحصول على النتائج

## التحضيرات المسبقة

قبل الاستخدام، تحتاج إلى الحصول على رمز API من XHuoAPI:

1. سجل أو قم بتسجيل الدخول إلى [منصة XHuoAPI](https://api.xhuoapi.ai)
2. انتقل إلى صفحة [Nano Banana Images API](https://api.xhuoapi.ai/documents/nano-banana-images)
3. انقر على "Acquire" للحصول على رمز API (تمنحك أول طلبات مجانية)

## التثبيت والتكوين

### الطريقة الأولى: تثبيت pip (موصى به)

```bash
pip install mcp-nanobanana-pro
```

### الطريقة الثانية: تثبيت من المصدر

```bash
git clone https://github.com/XHuoAPI/NanoBananaMCP.git
cd NanoBananaMCP
pip install -e .
```

بعد الانتهاء من التثبيت، يمكنك استخدام الأمر `mcp-nanobanana-pro` لبدء الخدمة.

## الاستخدام في Claude Desktop

قم بتحرير ملف تكوين Claude Desktop:

- **macOS**: `~/Library/Application Support/Claude/claude_desktop_config.json`
- **Windows**: `%APPDATA%\Claude\claude_desktop_config.json`

أضف التكوين التالي:

```json
{
  "mcpServers": {
    "nanobanana": {
      "command": "mcp-nanobanana-pro",
      "env": {
        "XHuoAPI_API_TOKEN": "رمز API الخاص بك"
      }
    }
  }
}
```

إذا كنت تستخدم `uvx` (لا حاجة لتثبيت الحزمة مسبقًا):

```json
{
  "mcpServers": {
    "nanobanana": {
      "command": "uvx",
      "args": ["mcp-nanobanana-pro"],
      "env": {
        "XHuoAPI_API_TOKEN": "رمز API الخاص بك"
      }
    }
  }
}
```

بعد حفظ التكوين، أعد تشغيل Claude Desktop، وستتمكن من استخدام أدوات Nano Banana في المحادثة.

## الاستخدام في VS Code / Cursor

قم بإنشاء ملف `.vscode/mcp.json` في جذر المشروع:

```json
{
  "servers": {
    "nanobanana": {
      "command": "mcp-nanobanana-pro",
      "env": {
        "XHuoAPI_API_TOKEN": "رمز API الخاص بك"
      }
    }
  }
}
```

أو استخدم `uvx`:

```json
{
  "servers": {
    "nanobanana": {
      "command": "uvx",
      "args": ["mcp-nanobanana-pro"],
      "env": {
        "XHuoAPI_API_TOKEN": "رمز API الخاص بك"
      }
    }
  }
}
```

## قائمة الأدوات المتاحة

| اسم الأداة                       | الوصف          |
| ---------------------------- | ----------- |
| `nanobanana_generate_image`  | توليد صورة من خلال نصوص توجيهية |
| `nanobanana_edit_image`      | تحرير أو دمج الصور الموجودة   |
| `nanobanana_get_task`        | استعلام عن حالة مهمة واحدة    |
| `nanobanana_get_tasks_batch` | استعلام عن حالة المهام بشكل جماعي    |

## أمثلة على الاستخدام

بعد الانتهاء من التكوين، يمكنك استدعاء هذه الوظائف مباشرةً بلغة طبيعية في عميل الذكاء الاصطناعي، مثل:

- "ساعدني في توليد صورة بأسلوب الألوان المائية"
- "قم بإضافة هذه الملابس إلى هذه الصورة"
- "ضع هذا المنتج في مشهد مقهى"
- "استخدم نموذج nano-banana-pro لتوليد صورة شخصية عالية الجودة"

## مزيد من المعلومات

- مستودع GitHub: [XHuoAPI/NanoBananaMCP](https://github.com/XHuoAPI/NanoBananaMCP)
- حزمة PyPI: [mcp-nanobanana-pro](https://pypi.org/project/mcp-nanobanana-pro/)
- وثائق API: [Nano Banana API](https://api.xhuoapi.ai/documents/nano-banana)
