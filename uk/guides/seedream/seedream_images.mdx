...
Щоб використовувати API, спочатку потрібно подати заявку на відповідний сервіс на сторінці [SeeDream Images Generation API](https://api.xhuoapi.ai/documents/seedream-images). Після переходу на сторінку натисніть кнопку «Acquire», як показано на зображенні:
