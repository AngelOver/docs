---
title: "Suno ανέβασμα αναφοράς ήχου API διασύνδεση οδηγίες"
description: "Suno Music Generation 集成指南 - XHuoAPI"
---

Το SUNO μας επιτρέπει να ανεβάσουμε αναφορά ήχου για δευτερεύουσα δημιουργία, αυτό το έγγραφο εξηγεί τη μέθοδο διασύνδεσης σχετικών API.

Αυτό το API έχει μόνο μία παράμετρο εισόδου, δηλαδή το `audio_url`, το οποίο είναι μια διεύθυνση CDN που μπορεί να προσπελαστεί δημόσια και υποστηρίζει κατάληξη mp3.

Εδώ η εισαγωγή μας για το `audio_url` είναι το `https://cdn.xhuoapi.ai/suno_demo.mp3`, που είναι μια διεύθυνση CDN που μπορεί να προσπελαστεί δημόσια.
...
Μπορούμε να δούμε ότι το πεδίο `audio_id` του `data` είναι το ID του τραγουδιού μετά την ανέβασή του.

Με το ID του τραγουδιού, μπορούμε να χρησιμοποιήσουμε το [Suno Audios Generation API](https://api.xhuoapi.ai/documents/4da95d9d-7722-4a72-857d-bf6be86036e9) για να δημιουργήσουμε προσαρμοσμένα τραγούδια, για παράδειγμα, αν περάσουμε το `action` ως `upload_extend`, ενώ το `audio_id` ως το επιστρεφόμενο ID του τραγουδιού, μπορούμε να δημιουργήσουμε νέο τραγούδι με βάση την αναφορά ήχου.
