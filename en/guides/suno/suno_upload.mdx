...
Here, the `audio_url` we input is `https://cdn.xhuoapi.ai/suno_demo.mp3`, which is a publicly accessible CDN address.
...
With the song ID, we can use the [Suno Audios Generation API](https://api.xhuoapi.ai/documents/4da95d9d-7722-4a72-857d-bf6be86036e9) to generate custom songs. For example, by passing `action` as `upload_extend` and `audio_id` as the returned song ID, we can generate a new song based on the reference audio.
