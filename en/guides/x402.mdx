...
This document is aimed at developers who need to integrate XHuoAPI platform x402 payment capabilities into their business, introducing the complete process from preparing the environment to completing payment calls, and providing example code for Axios, Fetch, Python requests, and httpx. Key points:
...
- All APIs are located under the domain `https://api.xhuoapi.ai`, and the `Authorization` header must carry the **Account Token** (previously called Platform Token).
...
Log in to the console at [https://api.xhuoapi.ai](https://api.xhuoapi.ai), where you can see the orders that need to be paid in the order list or order details page. The order details will display:
...
1. Open the console page [https://api.xhuoapi.ai/console/platform-tokens](https://api.xhuoapi.ai/console/platform-tokens);
...
- API base domain: `https://api.xhuoapi.ai`
...
- **Console Verification**: Visit the order details page `https://api.xhuoapi.ai/console/orders/{order_id}`, if the page shows "Payment Successful" or the order status has changed to Paid/Completed, it indicates that the on-chain settlement is complete.
- **API Verification**: Call `GET https://api.xhuoapi.ai/api/v1/orders/{order_id}/` with `Authorization: Bearer {platform_token}`, check the `state` field in the response (`PAID` or `FINISHED` indicates payment success).
...
- Submit tickets and customer support: [https://api.xhuoapi.ai/support](https://api.xhuoapi.ai/support)
- Community communication: Discord [https://discord.gg/f9GRuKCmRc](https://discord.gg/f9GRuKCmRc), X (Twitter) [https://x.com/XHuoAPI](https://x.com/XHuoAPI)
- Other channels: Email `office@xhuoapi.ai`, `office@germey.tech`; company address 651 N Broad St, Suite 201, Middletown, Delaware, USA; for WeChat customer service, please check the latest QR code on the support page.
