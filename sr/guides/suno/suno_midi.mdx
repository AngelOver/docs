---
title: "Suno MIDI Generacija API Uputstvo"
description: "Suno Music Generation 集成指南 - XHuoAPI"
---

SUNO nam omogućava da vršimo drugu kreativnu obradu generisane muzike, dobijajući MIDI muziku. Ovaj dokument objašnjava kako se povezati sa relevantnim API-jem.

Osnovni ulazni parametar ovog API-ja je `audio_id`, koji je ID pesme generisane od strane zvanične platforme, obično je to ID pesme sa svim trakama odvojenim (odgovara [generisanju pesama](https://api.xhuoapi.ai/documents/suno-audios) gde je parametar `action` postavljen na `all_stems`). Opcionalno, podržava i `callback_url` za asinkroni povratni poziv.

Ovde unosimo `audio_id` kao `c65e6ffd-ead3-4926-9c8c-a42ce202946b`.
...
Može se videti da su polja u `data` MIDI informacije pesme nakon odvajanja svih tonova.
