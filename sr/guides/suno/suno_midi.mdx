...
Osnovni ulazni parametar ovog API-ja je `audio_id`, koji je ID pesme generisane od strane zvanične platforme, obično je to ID pesme sa svim trakama odvojenim (odgovara [generisanju pesama](https://platform.acedata.cloud/documents/suno-audios) gde je parametar `action` postavljen na `all_stems`). Opcionalno, podržava i `callback_url` za asinkroni povratni poziv.
