...
Здесь мы вводим `audio_url`, который равен `https://cdn.xhuoapi.ai/suno_demo.mp3`, это общедоступный адрес CDN.
...
Получив ID песни, мы можем использовать [Suno Audios Generation API](https://api.xhuoapi.ai/documents/4da95d9d-7722-4a72-857d-bf6be86036e9) для генерации пользовательских песен, например, передав `action` как `upload_extend`, а `audio_id` как возвращенный ID песни, мы можем создать новую песню на основе справочного аудио.
