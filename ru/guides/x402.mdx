---
title: "XHuoAPI x402 Руководство по интеграции платежей"
description: "x402 集成指南 - XHuoAPI"
---

x402 — это протокол платежей на основе стандарта HTTP 402 Payment Required, который позволяет вызывающей стороне завершать расчет на блокчейне одновременно с отправкой API-запроса. Он идеально подходит для сценариев по оплате за вызовы между автоматизированными программами, AI Agent и микросервисами. С помощью этого руководства вы сможете программно осуществлять оплату заказов XHuoAPI в своей системе, реализуя опыт «запроса и оплаты».
...
- Все API находятся на домене `https://api.xhuoapi.ai`, заголовок `Authorization` должен содержать **Platform Token**.
...
Войдите в консоль [https://api.xhuoapi.ai](https://api.xhuoapi.ai), в списке заказов или на странице деталей заказа вы можете увидеть заказы, которые необходимо оплатить. В деталях заказа будет отображено:
...
1. Откройте страницу консоли [https://api.xhuoapi.ai/console/platform-tokens](https://api.xhuoapi.ai/console/platform-tokens);
...
- Базовый домен API: `https://api.xhuoapi.ai`
...
- **Проверка в консоли**：Перейдите на страницу деталей заказа `https://api.xhuoapi.ai/console/orders/{order_id}`. Если на странице отображается «Оплата успешна» или статус заказа изменился на оплаченный/завершенный, это означает, что расчет в цепочке завершен.
- **Проверка API**：Вызовите `GET https://api.xhuoapi.ai/api/v1/orders/{order_id}/` с заголовком `Authorization: Bearer {platform_token}` и проверьте поле `state` в ответе (значения `PAID` или `FINISHED` означают успешную оплату).
...
- Подать заявку и поддержка клиентов: [https://api.xhuoapi.ai/support](https://api.xhuoapi.ai/support)
- Сообщество: Discord [https://discord.gg/f9GRuKCmRc](https://discord.gg/f9GRuKCmRc), X (Twitter) [https://x.com/XHuoAPI](https://x.com/XHuoAPI)
- Другие каналы: электронная почта `office@xhuoapi.ai`, `office@germey.tech`; адрес компании 651 N Broad St, Suite 201, Middletown, Delaware, USA; для поддержки WeChat смотрите последние QR-коды на странице поддержки.
