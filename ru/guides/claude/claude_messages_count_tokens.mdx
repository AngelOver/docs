...
Чтобы использовать Claude Messages Count Tokens API, сначала перейдите на страницу [Claude Messages Count Tokens API](https://api.xhuoapi.ai/documents/claude-messages-count-tokens) и нажмите кнопку «Acquire», чтобы получить необходимые для запроса учетные данные.
