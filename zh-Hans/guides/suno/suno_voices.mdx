...
有了声音角色 ID 之后，我们便可以使用 [Suno Audios Generation API](https://api.xhuoapi.ai/documents/4da95d9d-7722-4a72-857d-bf6be86036e9) 来进行音乐生成了。将 `action` 设为 `generate`，并将 `persona_id` 设为上面返回的声音角色 ID，生成的歌曲将使用克隆的声音进行演唱。
