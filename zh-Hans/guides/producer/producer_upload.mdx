...
这里我们输入的 `audio_url` 是 `https://cdn.xhuoapi.ai/suno_demo.mp3`，是一个可以公开访问的 CDN 地址。
...
有了歌曲 ID 之后，我们便可以使用 [Producer Audios Generation API](https://api.xhuoapi.ai/documents/producer-audios-integration) 来进行自定义歌曲生成了，比如说 `action` 传入 `upload_extend`，同时 `audio_id` 传入返回的歌曲 ID，就可以根据参考音频生成新的歌曲了。
