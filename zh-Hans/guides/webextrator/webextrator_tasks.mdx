...
`POST https://api.xhuoapi.ai/v1/webextrator/tasks`
...
```bash
curl -X POST https://api.xhuoapi.ai/v1/webextrator/tasks \
...
app.post('/hooks/webextrator', async (req, res) => {
  ...
  const fetchRes = await fetch('https://api.xhuoapi.ai/v1/webextrator/tasks', {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${process.env.XHuoAPI_API_KEY}`,
      'Content-Type': 'application/json',
    },
