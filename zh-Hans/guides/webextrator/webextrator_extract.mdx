...
`POST https://api.xhuoapi.ai/v1/webextrator/extract`
...
[WebExtrator 服务页](https://api.xhuoapi.ai/service/webextrator)
...
```bash
curl -X POST https://api.xhuoapi.ai/v1/webextrator/extract \
...
API_KEY = os.environ["XHuoAPI_API_KEY"]

resp = requests.post(
    "https://api.xhuoapi.ai/v1/webextrator/extract",
    headers={
...
```js
const apiKey = process.env.XHuoAPI_API_KEY;

const res = await fetch('https://api.xhuoapi.ai/v1/webextrator/extract', {
  method: 'POST',
  headers: {
